<template>
  <div id="index" ref="appRef">
    <div class="bg">
      <dv-loading v-show="loading">载入中</dv-loading>
      <div class="host-body">
        <div>
          <!-- 顶部title部分 -->
          <el-row>
            <el-col :span="6">
              <router-link to="/screen_dj">
                <div class="title_text2">党建管理</div>
              </router-link>
              <dv-decoration-8 class="title_right" :color="['#008CFF', '#00ADDD']" /></el-col>
            <el-col :span="12">
              <router-link to="/login">
                <div class="title_text1">小镇数字化指挥中心</div>
              </router-link>
          ...
          <el-row>
            ...
            <el-col :span="6">
              ...
              <div class="left_box1">
                <dv-border-box-12>
                  ...
                  <div v-for="(item, index) in numberData" :key="index" class="rose_text">
                    <p>
                      <span class="coin">数量</span>
                    ...
                    <p>
                      <span>{{ item.text }}</span>
                      <span class="colorYellow">(今日)</span>
                    </p>
                  </div>
                </dv-border-box-12>
              </div>
              ...
              <div class="left_box2">
                <dv-border-box-12 style="padding-top: 10px">
                  <p style="margin-left: 15px">乡村公路统计图</p>
                  <div id="columnar" />
                </dv-border-box-12>
              </div>
              <!-- 轮播表格部分 -->
              <div class="left_box3">
                <dv-border-box-12 style="padding-top: 10px">
                  <dv-scroll-board :config="board_info" class="carousel_list" odd-row-b-g-c="#fff" />
                </dv-border-box-12>
              </div>
            </el-col>
            <!-- 第二列 -->
            ...
            <el-col :span="6">
              ...
              <div class="right_box1">
                <dv-border-box-12>
                  <dv-decoration-7 style="width: 100%; height: 30px">乡 村 收 入 排 行 榜</dv-decoration-7>
              ...
              <div class="right_box3">
                <dv-border-box-12 :reverse="true">
                  <dv-capsule-chart :config="cone" class="cone_box" style="width:300px;height:300px" />
                </dv-border-box-12>
              </div>
            </el-col>
          </el-row>
...
import { formatTime } from '@/utils/index2.js' // 日期格式转换
...
export default {
  ...
  methods: {
    ...
    // 柱状图
    columnar() {
      const mapChart = this.$echarts.init(document.getElementById('columnar')) // 图表初始化，china-map是绑定的元素
      window.onresize = mapChart.resize // 如果容器变大小，自适应从新构图
      const option = {
        title: {
          text: ''
    ...
    // 右侧虚线柱状图图
    dotter_bar() {
      const mapChart = this.$echarts.init(document.getElementById('dotter_bar')) // 图表初始化，china-map是绑定的元素
      window.onresize = mapChart.resize // 如果容器变大小，自适应从新构图
      // Generate data
      const category = []
      ...
      for (let i = 0; i < 20; i++) {
        ...
        const b = parseInt(Math.random() * 2000)
        const d = parseInt(Math.random() * 2000)
        barData.push(b)
        lineData.push(d + b)
      }
      // option
...
#index {
  ...
  .bg {
    ...
    background-image: url('~@/assets/backimages/back2.png'); //背景图
